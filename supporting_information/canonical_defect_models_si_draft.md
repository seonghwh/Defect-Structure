...
The model equations were implemented in Python. The final model is contained in `src/batio3_defects/solver.py`. The defect diagrams corresponding to manuscript Figure 6 and the exact air-point table are generated by

```text
python scripts/generate_manuscript_outputs.py
```

This script writes the plotted data to `results/figure6_defect_concentrations_data.csv`, the exact air-point table to `results/table3_air_values.csv` and `results/table3_air_values.md`, and the figure files to `figures/figure6_defect_concentrations.png`, `.pdf`, and `.svg`.
